...
score by changing the different parameters, that is C and gamma. In addition,
it will draw a heatmap of insampe and outsample accuracy scores by different 
parameters. Lastly, it will print the best insample acccuracy score, outsample
accuracy score, and the running time. 


==========================
Explain RBF SVM parameters
==========================
source: 
...
# To use functions from BoW_Naive_Bayes.py
...
def BoW_Support_Vector_Machines(sample_size = 100, nth_day = 5,\
    ...
    # Use svm to fit data, the default setting will use RBF kernal
    clf = svm.SVC().fit(count_train, y_train)
    #print(clf)
...
def BoW_Support_Vector_Machines_plot(sample_size = 100, nth_day = 5,\
    ...
    for C in C_range:
        for gamma in gamma_range:
            # Use svm to fit data, the default setting will use RBF kernal
            clf = svm.SVC(C=C,gamma=gamma).fit(count_train, y_train)
            #print(clf)
            insample_pred=clf.predict(count_train)
            pred = clf.predict(count_test)
            outsample_accuracy_score = metrics.accuracy_score(y_test, pred)   
